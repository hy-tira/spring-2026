...
### Related notions
For our purposes, the notion of $$O(\cdots)$$ is sufficient, but for more refined analysis of algorithms, it is of interest to study the symmetric lower-bound measure of how many steps the algorithm at least takes, denoted  $$\Omega(\cdots)$$. If an algorithm has the same $$O(\cdots)$$ and $$\Omega(\cdots)$$ complexities, it is denoted to work in time  $$\Theta(\cdots)$$. Often also notions $$o(\cdots)$$ and $$\omega(\cdots)$$ become handy, which are denoting strictly smaller and larger complexities, respectively. E.g., $$n=o(n\log\log n)$$ and $$n=\omega(n\log\log n/\log n)$$.
